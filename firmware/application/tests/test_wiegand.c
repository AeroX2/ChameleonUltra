...
static void test_hint_and_capacity(void) {
    ...
    wiegand_card_t *selected = unpack(IND26, 26, 0, raw);
    assert(selected != NULL);
    assert(selected->format == IND26);
    assert(!selected->parity_valid);
    free(selected);
    assert(unpack_all(0, 25, 0, raw, &candidate, 1) == 0);
}

static void test_invalid_parity_is_not_h10301_specific(void) {
    wiegand_card_t source = {
        .facility_code = 42,
        .card_number = 1234,
        .format = W2804,
    };
    uint64_t raw = pack(&source) ^ 1ULL;
    wiegand_candidate_t candidate = {0};

    assert(unpack_all(W2804, 28, 0, raw, &candidate, 1) == 1);
    assert(candidate.card.format == W2804);
    assert(candidate.card.facility_code == source.facility_code);
    assert(candidate.card.card_number == source.card_number);
    assert(candidate.flags & WIEGAND_CANDIDATE_HAS_PARITY);
    assert(!(candidate.flags & WIEGAND_CANDIDATE_PARITY_VALID));

    wiegand_card_t *selected = unpack(W2804, 28, 0, raw);
    assert(selected != NULL);
    assert(selected->format == W2804);
    assert(!selected->parity_valid);
    free(selected);
}
...
int main(void) {
    ...
    test_invalid_parity_is_not_h10301_specific();
